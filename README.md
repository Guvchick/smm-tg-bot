...
- Заказ через платформы и подкатегории SocRocket: VK отдельно, YouTube отдельно, Telegram отдельно, затем выбор лота и обычный/массовый режим.
...
- Профиль пользователя: пополнение, рефералы, промокод, последние заказы и пополнения.
- Админ-панель на inline-кнопках: статистика, пользователи, поиск, оплаты, промокоды, рассылки, бэкапы и восстановление.
- Раздел поддержки с обращениями в админ-группу/топик.
...
- Ручная отправка и восстановление PostgreSQL-бэкапа из `.sql` или `.zip`.
- Фоновая проверка статусов заказов и уведомления покупателю.
- Фоновая проверка ожидающих CryptoBot-счетов через polling, если webhook не дошел.

## Быстрый старт

1. Установить Go 1.25+, Docker, `pg_dump`.
...
Основные админ-действия также доступны кнопками в разделе `🛠 Админ`.
...
- `ADMIN_TOPIC_*_ID` — ID топиков для заказов, оплат, бэкапов и поддержки.
- `SUPPORT_TEXT`, `SUPPORT_URL`, `SUPPORT_USERNAME` — раздел поддержки.
- `NEW_USER_BONUS_RUB` — стартовый бонус новым пользователям.
- `DEFAULT_MARKUP_PERCENT` — базовая наценка, если у услуги нет своей.
- `REFERRAL_PERCENT` — бонус рефереру от успешного пополнения.
- `PAYMENT_POLL_ENABLED`, `PAYMENT_POLL_INTERVAL` — автоматическая проверка ожидающих CryptoBot-счетов.
- `GOOGLE_SHEETS_ENABLED`, `GOOGLE_SHEETS_SPREADSHEET_ID`, `GOOGLE_SHEETS_CREDENTIALS_FILE`/`GOOGLE_SHEETS_CREDENTIALS_JSON` — запись заказов и пополнений в Google Sheets.
